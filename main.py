...
def poisson_points(intensity,radius): # intensity, radius of the disk of sampling in hyperbolic metric
    ...
    radii = np.tanh(np.arccosh(1 + radii_coordinates*(np.cosh(radius)-1))) # sample radii by inverse transform sampling
...
def plot_PVT(intensity,radius,density):
    r = np.linspace(0, np.tanh(radius/2), density) #radius in poincare disk
